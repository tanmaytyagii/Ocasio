  ...
  if (loading) {
    return (
      <section className="mt-8 rounded-lg bg-white p-8 shadow-sm">
        ...
        <div className="h-16 w-full animate-pulse rounded bg-gray-200" aria-hidden="true" />
      </section>
    );
  }
  ...
  return (
    <section className="mt-8 rounded-lg bg-white p-8 shadow-sm">
      ...
      {payment && (
        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            ...
            <dd data-testid="payment-amount" className="mt-1 font-medium text-gray-900">
              {formatMinor(payment.amount_minor, payment.currency)}
            </dd>
          </div>
          {payment.amount_refunded_minor > 0 && (
            <div>
              <dt className="text-sm text-gray-500">Refunded</dt>
      ...
      {payment?.status === 'processing' && (
        <div className="mt-6 flex gap-3 rounded-lg border border-amber-300 bg-amber-50 p-4" role="note">
      ...
      {isCustomer && payment && (payment.status === 'pending' || payment.status === 'processing') && (
        <div className="mt-4">
          <button
            ...
            disabled={busy}
            className="text-sm text-gray-600 underline hover:text-gray-900 disabled:opacity-60"
          >
      ...
      {refunds.length > 0 && (
        <div className="mt-6 border-t pt-6">
          <h3 className="mb-3 text-sm font-medium text-gray-500">Refunds</h3>
            ...
            {refunds.map((refund) => (
              <li key={refund.id} className="flex items-center justify-between text-sm">
                <span className="text-gray-900">
                  ...
                  {refund.reason && <span className="text-gray-600"> · {refund.reason}</span>}
                </span>
                <span className="text-gray-500">{formatTimestamp(refund.created_at)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {showRefundForm && payment && (
        <div className="mt-6 border-t pt-6">
          <h3 className="mb-3 text-sm font-medium text-gray-900">Issue a refund</h3>
          <p className="mb-3 text-xs text-gray-500">
            Refundable balance: {formatMinor(refundableBalanceMinor(payment), payment.currency)}
          </p>
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="sm:w-40">
              <label htmlFor="refund-amount" className="mb-1 block text-sm text-gray-700">
                Amount (₹)
              </label>
              <input
                id="refund-amount"
                ...
                className="w-full rounded-lg border border-gray-300 p-2 text-sm focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-600"
              />
            </div>
          ...
          <button
            ...
            disabled={busy}
            className="mt-3 inline-flex items-center gap-2 rounded-lg border border-red-300 px-4 py-2 text-sm text-red-700 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {busy ? (
              <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />
            ) : (
