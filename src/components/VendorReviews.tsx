...
const VendorReviews = ({
  ...
  return (
    <div className="mt-10 border-t pt-8">
      ...
      {ratingIsDemo && (
        <p className="mb-4 rounded-card border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
      ...
      {loading && (
        <div className="space-y-3" aria-hidden="true">
          {Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="h-16 w-full animate-pulse rounded-lg bg-canvas" />
          ))}
        </div>
      )}
